...
fn discord_message_detail(query: &OrientQuery<'_>, anchor: Id) -> Result<DiscordMessageDetail> {
    ...
    for observation in newest {
        ...
        for (attachment, name) in find!(
            ...
            pattern!(query.discord, [
                { observation @ archive::attachment: ?attachment },
        ...
        attachments.sort();
        versions.insert(DiscordVersion {
            content,
            attachments,
        });
    }
...
fn discord_news_line(query: &OrientQuery<'_>, anchor: Id) -> Option<String> {
    ...
    let attachments = detail
        .versions
        .iter()
        .flat_map(|version| &version.attachments)
        .collect::<BTreeSet<_>>()
        .len();
    match attachments {
        0 => {}
    ...
    if !new_discord.is_empty() {
        ...
        for message in &new_discord {
            ...
            for version in &detail.versions {
                ...
                .unwrap();
                // The bytes are in the pile, stored with the message; `files
                // get` with `@-` writes them out by content hash.
                for (name, content) in &version.attachments {
                    match content {
