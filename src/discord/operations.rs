...
impl Discord {
    ...
    /// nothing at or below it is stored, on the forward read or the recent
    /// page alike, and the forward read goes on from as far as coverage
    /// reaches from the floor, whatever older coverage the channel has; with
    /// none past the floor it begins just after the floor instead of taking
    /// the newest page as a bounded baseline.
    pub fn pull_channel(
        &self,
        channel_id: &str,
        floor: Option<u64>,
...
pub trait Source {
    ...
    /// One message by its id, as `GET /channels/{id}/messages/{message}`
    /// returns it; None only when Discord answers that there is no such
    /// message (Unknown Message). Any other refusal, a permission the bot
    /// lacks included, is an error.
    fn message(&mut self, channel_id: &str, message_id: u64) -> Result<Option<JsonValue>>;
...
impl Source for Rest {
    ...
    fn message(&mut self, channel_id: &str, message_id: u64) -> Result<Option<JsonValue>> {
        fetch_message(&self.token, channel_id, message_id)
    }
...
fn pull_channel(
    ...
    let intervals = discord_model::channel_intervals(&view.facts, channel)?;
    // With a floor, coverage goes on from as far as it reaches from the
    // floor, whatever older coverage the channel has, and begins there as a
    // baseline when it reaches no further. Without one, it goes on from its
    // connected frontier, and a channel without any begins as a bounded
    // baseline. Recent edits are reconciled once something is covered.
    let (after, baseline, reconcile) = match floor {
        Some(floor) => {
            let reached = discord_model::reach(&intervals, floor);
            (Some(reached), reached == floor, reached > floor)
        }
        None => match discord_model::connected_frontier(&intervals) {
            Some(coverage) => (Some(coverage.through_inclusive), false, true),
    ...
    let recent_payloads = if reconcile {
        source.page(
            channel_id,
            PageRequest {
                after: None,
    ...
    let mut messages = parse_messages(payloads, channel_id)?;
    // Nothing at or below the floor is kept: the recent page reaches back
    // past it in a quiet channel.
    if let Some(floor) = floor {
        messages.retain(|message| {
            discord_model::validate_snowflake(&message.external_id)
    ...
    let fragment =
        build_ingest_fragment(&messages, forward.coverage, Some(&*view), |url, limit| {
            source.attachment(url, limit)
        })?;
...
/// Discord's JSON error code for a message that does not exist.
const UNKNOWN_MESSAGE: i64 = 10008;

/// One message by its id: None only when Discord answers 404 with Unknown
/// Message. A channel the bot may not read answers 403, and a page of it
/// comes back empty rather than refused, so neither says the message is gone.
fn fetch_message(token: &str, channel_id: &str, message_id: u64) -> Result<Option<JsonValue>> {
    ...
    if status.is_success() {
        return response.json().map(Some).context("parse Discord message");
    }
    let body = response.text().unwrap_or_default();
...
fn unknown_message(status: u16, body: &str) -> bool {
    status == 404
...
/// attachment `stored` already holds the bytes of is built from those bytes
/// instead of fetched again, into the very facts a download would give, so
/// every observation carries its attachments whole: a reader that has this
/// fragment and not the earlier one still names every attachment. One
/// declared larger than [`MAX_ATTACHMENT_BYTES`] is recorded by name and size
/// alone.
fn build_ingest_fragment<F>(
    messages: &[IncomingMessage],
    coverage: Option<discord_model::CoverageInterval>,
    stored: Option<&CollectionView>,
    mut fetch: F,
) -> Result<Fragment>
where
    F: FnMut(&str, u64) -> Result<Vec<u8>>,
{
    ...
    for (key, transport) in transports {
        ...
        // The same Discord attachment, stored with its bytes by an earlier
        // pull, whose bytes this pile holds: built from those.
        let mut bytes = stored.and_then(|view| {
            ...
            find!(
                content: file_capability::ContentHandle,
                pattern!(&view.facts, [
                    {
                        _?attachment @
                        metadata::tag: archive::kind_attachment,
                    ...
                    { _?file @ file::content: ?content },
                ])
            )
            .find_map(|content| view.reader.get::<Bytes, RawBytes>(content).ok())
        });
        ...
        for url in &transport.urls {
            ...
            match fetch(url, MAX_ATTACHMENT_BYTES) {
                Ok(value) => bytes = Some(Bytes::from_source(value)),
...
mod tests {
    ...
    /// Only a 404 carrying Discord's Unknown Message code says a message
    /// does not exist; a missing permission, an unknown channel, a rate limit
    /// or an answer that is not Discord's error object do not.
    #[test]
    fn only_unknown_message_says_a_message_is_gone() {
        assert!(unknown_message(
            404,
            r#"{"message": "Unknown Message", "code": 10008}"#
        ));
        for (status, body) in [
            (403, r#"{"message": "Missing Permissions", "code": 50013}"#),
            (403, r#"{"message": "Missing Access", "code": 50001}"#),
            (404, r#"{"message": "Unknown Channel", "code": 10003}"#),
            (404, "<html>not found</html>"),
            (404, ""),
            (
                429,
                r#"{"message": "You are being rate limited.", "code": 0}"#,
            ),
            (500, r#"{"message": "Unknown Message", "code": 10008}"#),
        ] {
            assert!(!unknown_message(status, body), "{status} {body}");
        }
    }
    ...
    /// An attachment an earlier pull stored is not downloaded again, and the
    /// new observation still carries it whole: a reader that has only the
    /// new one (a replica it reached first) names the attachment and its
    /// stored content, exactly as after a download.
    #[test]
    fn a_stored_attachment_is_rebuilt_from_its_bytes_so_every_observation_names_it() {
        let directory = tempfile::tempdir().unwrap();
        let (pile, key) = fresh_storage(&directory);
        let owner = crate::storage::Storage::new(pile.clone(), Some(key.clone()));
        let storage = test_storage(&owner);
        let channel = "100000000000000014";
        let payload = |url: &str, content: &str, edited: Option<&str>| {
            message_json(
                "100000000000000015",
                channel,
                content,
        ...
        let downloaded = build_ingest_fragment(
            &parse_messages(
                vec![payload("https://cdn.example/a", "a photo", None)],
                channel,
            )
            .unwrap(),
            None,
        ...
        storage
            ...
            .unwrap();
        let view = storage.view().unwrap();
        let rebuilt = build_ingest_fragment(
            &parse_messages(
                vec![payload("https://cdn.example/b", "a photo", None)],
                channel,
            )
            .unwrap(),
            None,
        ...
        .unwrap();
        // The same observation, attachment and file a download gives.
        assert_eq!(rebuilt, downloaded);

        // An edit observed later, alone, is just as complete: orient's
        // attachment join finds the name and the stored content in it.
        let edit = payload(
            "https://cdn.example/c",
            "a photo, edited",
            Some("2026-08-07T09:00:00Z"),
        );
        let edited = build_ingest_fragment(
            &parse_messages(vec![edit.clone()], channel).unwrap(),
            None,
            Some(&view),
            |_, _| unreachable!("already stored"),
        )
        ...
        assert_eq!(
            edited,
            build_ingest_fragment(
                &parse_messages(vec![edit], channel).unwrap(),
                None,
                None,
                |_, _| Ok(b"image".to_vec()),
            )
        ...
        let photo: file_capability::ContentHandle = b"image".to_vec().to_blob().get_handle();
        let named = find!(
            (name: discord_model::TextHandle, content: file_capability::ContentHandle),
            pattern!(&edited, [
                {
                    _?observation @
                    metadata::tag: archive::kind_message,
                    archive::attachment: _?attachment,
                },
                { _?attachment @ archive::attachment_name: ?name },
                {
                    _?attachment @ archive::attachment_file: _?file
                },
                { _?file @ file::content: ?content },
        ...
        .collect::<Vec<_>>();
        let photo_name: discord_model::TextHandle = "photo.png".to_owned().to_blob().get_handle();
        assert_eq!(named, [(photo_name, photo)]);
        assert!(edited
            .blobs()
            .clone()
            .snapshot()
            .unwrap()
            .get::<Bytes, RawBytes>(photo)
            .is_ok());

        // A replica that has the edit before the first observation.
        let replica_directory = tempfile::tempdir().unwrap();
        let (pile, key) = fresh_storage(&replica_directory);
        let owner = crate::storage::Storage::new(pile, Some(key));
        let replica = test_storage(&owner);
        replica.publish(edited, "edit first".to_owned()).unwrap();
        let history = read_history(&replica.view().unwrap(), &ReadOptions::default()).unwrap();
        assert_eq!(history.messages.len(), 1);
        assert_eq!(history.messages[0].content, "a photo, edited");
        assert_eq!(history.messages[0].attachments.len(), 1);
    }
