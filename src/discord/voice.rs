...
async fn run(
    ...
    let mut link = Link::new(connected);
...
impl Link {
    fn new(connected: watch::Sender<bool>) -> Self {
        Self {
            connected,
            join: Rejoin::default(),
            attempt: 0,
    ...
    /// A new connection, as the attempt to hand the driver.
...
async fn speak_line(voice: &mut Voice, speech: &SpeechWorker, queued: &Path) -> Spoken {
    ...
    let ended = tokio::select! {
        ...
        // ending it.
        _ = voice.connected.wait_for(|up| !*up) => Ended::Disconnected,
    };
    let played = started.elapsed().as_secs_f64();
    let interrupted = voice.interruptions.load(Ordering::SeqCst) != interruptions;
...
enum Ended {
    ...
    TimedOut,
    /// The voice connection stopped working.
    Disconnected,
}
