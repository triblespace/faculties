...
//! channel from the earliest message heard in it as new: the state directory
//! keeps that floor for each, and nothing at or below it is stored, whether a
//! pull or the gateway brings it (an edit of an older message, the recent
//! page a backfill reconciles), since a channel's history would reach every
//! window as news. A floor that is there but cannot be read passes its
//! channel over rather than reading its history. `discord read` is how
//! history comes in when it is wanted.
//!
...
//! fetches it again by that id until it is stored, Discord answers that it
//! does not exist or its channel is no longer intake's: the recent page a
//! backfill reconciles need not reach back to it.
//!
...
/// Where intake keeps the messages whose live write failed, one empty file
/// each: `<channel>-<message>`, and `<channel>-<message>-new` ([`NEW_DM`])
/// for a new message in a DM channel. The name is the whole record, so no
/// marker is ever half kept: a kind written into the file after creating it
/// could be cut short by a full disk or a crash, and the empty marker left
/// would be taken for an edit's and let go once a later message began its
/// channel.
...
pub struct Intake {
    ...
    /// for each message whose live write failed (`-new` after it for a new
    /// message in a DM channel).
    directory: PathBuf,
...
impl Intake {
    ...
    /// Where a message whose live write failed is kept, and whether it was
    /// kept as a new message in a DM channel.
    ...
    /// not begin as one.
    ///
    /// The kind is in the marker's name, and creating the empty file is all
    /// keeping writes: a marker is there whole or not at all. A message kept
    /// both ways is fetched again as new.
    ...
    /// Keep a message no longer, either way it was kept: it is stored, or let
    /// go.
    ...
    /// channel's (with DMs asked for), one below the earliest message heard
    /// in it as new.
    ///
    /// `first` is one below a message heard as new: a MESSAGE_CREATE, live
    /// or kept by id and fetched again. In a DM channel it is kept as the
    /// floor when there is none, and lowers a floor above it, which a later
    /// message set because it was handled first (the gateway's order, or a
    /// later kept message fetched again before an earlier one, or stored live
    /// before the earlier one is fetched again). Everything that lowering
    /// lets in was sent after a message heard as new, so none of it is the
    /// channel's history, and every message heard as new is stored whatever
    /// the order. Nothing else moves a floor: not an edit, not a pull, and
    /// never a configured channel's.
    ...
    /// Keep `floor` as where `channel` begins under `kind`.
    ...
    /// store it, oldest first in each channel; one Discord answers does not
    /// exist, or that is not intake's to store, is let go. How many are
    /// still not stored.
    fn retry_unstored(&mut self) -> usize {
        ...
        // Each kept message once, in id order within each channel, and new
        // when either of its markers says so.
    ...
    /// One message fetched again by its id and stored as the gateway's
    /// would be: Ignored when it is not intake's to store or Discord answers
    ...
    fn refetch(&mut self, channel: u64, id: u64, new_dm: bool) -> Result<Done> {
        ...
        // Only Discord's answer that the message does not exist lets it go.
        // An empty page is no such answer: it is also what a channel whose
        // history the bot may not read gives, and the message fetched by id
        // is then refused, an error that keeps it.
...
mod tests {
    ...
    /// URL. Every page request and every message fetched by id is recorded.
    /// A channel whose history the bot may not read (`unreadable`) answers
    /// as Discord does: its pages are empty, and a message fetched by id is
    /// refused.
    ...
    /// The review's R1: a DM channel begins with the earliest message heard
    /// in it as new, whatever order its messages are handled in. Two first
    /// messages kept because no floor could be kept, the later one fetched
    /// again first; one kept while a later one is then stored live; and live
    /// messages the gateway delivers out of order. Every one is stored, the
    /// channel's older history none.
    #[test]
    fn the_earliest_new_message_of_a_dm_channel_begins_it_in_any_order() {
        ...
        // Discord's message ids are unique across channels: each DM channel
        // here has its own, `base` and `base + 1`, and an old DM before
        // them that is its history.
        ...
        // Both first messages kept: no DM floor can be kept while the DM
        // state is a file where its directory belongs.
        let dm = CHANNEL + 2;
        ...
        std::fs::remove_file(&dms).unwrap();
        // The later one is fetched again first: the earlier one's marker is
        // not there yet.
        let earlier = unstored.join(format!("{dm}-{}-{NEW_DM}", FLOOR + 170));
        let aside = fixture.state.join("aside");
        std::fs::rename(&earlier, &aside).unwrap();
        assert_eq!(intake.handle(Work::Backfill).unwrap(), caught_up(0));
        ...
        // The first message kept, and the next one stored live before a
        // backfill fetches the first again.
        let dm = CHANNEL + 3;
        ...
        // Live messages out of order: the later one first.
        let dm = CHANNEL + 4;
        ...
        // An edit is not heard as new: it lowers no floor.
    ...
    /// A kept new message of a DM channel is marked so by its marker's name,
    /// on an empty file: nothing written after the file is created, which a
    /// full disk or a crash could cut short, decides it. So it still begins
    /// its channel once a later message, stored live first, set the floor
    /// above it; and an edit of it kept as well does not unmark it.
    #[test]
    fn a_kept_new_dm_is_marked_by_its_name_alone() {
        ...
        // No DM floor can be kept while the DM state is a file where its
        // directory belongs: the first message is kept as new, and an edit
        // of it is kept as well.
        std::fs::create_dir_all(&fixture.state).unwrap();
        std::fs::write(&dms, "").unwrap();
        ...
        // Repaired, and the next message is stored live before any backfill:
        // it sets the floor at the kept one.
        std::fs::remove_file(&dms).unwrap();
        assert_eq!(intake.handle(Work::Message(second)).unwrap(), Done::Stored);
        let floor_of = || {
            std::fs::read_to_string(dms.join(dm.to_string()))
        ...
        // Fetched again as new, the kept one lowers the floor and is stored;
        // both markers are gone, and none of the history is stored.
        assert_eq!(
            intake.handle(Work::Backfill).unwrap(),
            Done::Backfilled {
                channels: 1,
    ...
    /// The review's R2: Discord answers a channel whose history the bot may
    /// not read with empty pages, so an empty answer does not say a kept
    /// message is gone. While the permission is missing the message stays
    /// kept and the backfill has not caught up; once it is back, the message
    /// is fetched by its id and stored, though no page a backfill reads
    /// reaches back to it.
    #[test]
    fn a_kept_message_outlives_a_channel_whose_history_cannot_be_read() {
        ...
        // An edit of an older message whose live write fails.
        let mut edited = rest(
            &(FLOOR + 101).to_string(),
            CHANNEL,
            "m101, edited",
        ...
        // Read Message History is taken away.
        fixture
            .fake
            .unreadable
            .lock()
